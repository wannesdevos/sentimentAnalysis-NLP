...
def benchmark_model(predict_fn, model_name, df):
    ...
    precision, recall, f1, _ = precision_recall_fscore_support(
        df["sentiment"], predictions, average="binary", pos_label="positive"
    )
    ...
    output_file = (
        f"./output/{model_name.replace(' ', '_').replace('/', '-')}_results.csv"
    )
...
def main():
    ...
    # Model 1: DistilBERT
    results.append(benchmark_model(predict_model_1, "Model 1 - DistilBERT", df))

    # Model 2: RoBERTa Large
    results.append(benchmark_model(predict_model_2, "Model 2 - RoBERTa Large", df))
    ...
    print(f"{'=' * 60}")
    print(
        f"Best Accuracy:  {best_accuracy['model_name']} ({best_accuracy['accuracy']:.4f})"
    )
    print(f"Best F1-Score:  {best_f1['model_name']} ({best_f1['f1_score']:.4f})")
    print(
        f"Fastest:        {fastest['model_name']} ({fastest['avg_time_per_review']:.4f}s/review)"
    )
